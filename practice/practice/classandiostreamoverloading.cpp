...
public:
	...
	friend std::ostream& operator<<(std::ostream& os, Rect& p);//friend라는 것만 명시
};

std::ostream& operator<<(std::ostream& os, Rect& p)//이게 진짜 함수
{
	os << p.position.x << " " << p.position.y << " " << p.w << " " << p.h << std::endl;
	return os;
}
